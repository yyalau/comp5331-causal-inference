...
class ERMDataModule(BaseDataModule):
    ...
    def val_dataloader(self) -> DataLoader[DatasetOutput]:
        return DataLoader(self.val_ds, batch_size=self.max_batches, shuffle=True, collate_fn=self.val_ds.collate_erm)

    def test_dataloader(self) -> DataLoader[DatasetOutput]:
        return DataLoader(self.test_ds, batch_size=self.max_batches, collate_fn=self.test_ds.collate_erm)

    def predict_dataloader(self) -> DataLoader[DatasetOutput]:
        return DataLoader(self.full_ds, batch_size=self.max_batches, collate_fn=self.full_ds.collate_erm)
